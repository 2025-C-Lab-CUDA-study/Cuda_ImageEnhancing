...
constexpr int BLUR_INTENSITY = 200;
...
//        kawaseBlur <<<kawaseGrid, block>>> (d_rBuf, pitch, offset, width, height);
//        if (!CudaKernelCheck()) { goto LB_RETURN; }
//        kawaseBlur <<<kawaseGrid, block>>> (d_gBuf, pitch, offset, width, height);
//        if (!CudaKernelCheck()) { goto LB_RETURN; }
//        kawaseBlur <<<kawaseGrid, block>>> (d_bBuf, pitch, offset, width, height);
//        if (!CudaKernelCheck()) { goto LB_RETURN; }
//        
//        ++offset;
//    }
...
//    { 
//        dim3 grid((width + BLOCK - 1) / BLOCK, (width + BLOCK - 1) / BLOCK);
//        bilinearIncrease <<<grid, block>>> (d_rBuf, pitch, width, height);
//        if (!CudaKernelCheck()) { goto LB_RETURN; }
//        bilinearIncrease <<<grid, block>>> (d_gBuf, pitch, width, height);
//        if (!CudaKernelCheck()) { goto LB_RETURN; }
//        bilinearIncrease <<<grid, block>>> (d_bBuf, pitch, width, height);
//        if (!CudaKernelCheck()) { goto LB_RETURN; }
//
//        width *= 2;
//        height *= 2;
...
__global__ void bilinearReduce(uint8_t* dst, uint8_t* src, size_t pitch, uint32_t width, uint32_t height)
{
    ...
    temp[tx][ty] = src[srcX + pitch * srcY];
    ...
    if (etx > BLOCK - 2 || ety > BLOCK - 2) return;

    uint8_t tl = temp[etx][ety];
    uint8_t tr = temp[etx][ety + 1];
    uint8_t bl = temp[etx + 1][ety];
    uint8_t br = temp[etx + 1][ety + 1];

    uint8_t val = ((tl * (1 - dx) + tr * dx) * (1 - dy) + (bl * (1 - dx) + br * dx) * dy) * 0.25;

    if (val > 255 || val < 0) return;

    uint32_t outX = srcX / 2;
    uint32_t outY = srcY / 2;

    if (outX > reduceW - 1 || outY > reduceH - 1 || outX < 0 || outY < 0) return;

    dst[outY * pitch + outX] = val;
}

__global__ void bilinearIncrease(uint8_t* dst, uint8_t* src, size_t pitch, uint32_t width, uint32_t height)
{
    // TODO : (IMP) use shared memory and do not exceed widht and height ( srcx and y )
    __shared__ uint8_t temp[BLOCK][BLOCK];
    ...
    if (srcX > width - 1 || srcY > height - 1 || srcX < 0 || srcY < 0) return;
    ...
    uint8_t tl = temp[ix][iy];
    uint8_t tr = temp[x ][iy];
    uint8_t bl = temp[iy][y ];
    uint8_t br = temp[x ][y ];

    float v = ((tl * (1 - dx) + tr * dx) * (1 - dy) +
        (bl * (1 - dx) + br * dx) * dy) * 0.25;

    if (v > 255 || v < 0) return;

    uint8_t outV = (uint8_t)v;

    uint32_t outX = srcX * 2;
    uint32_t outY = srcY * 2;

    if (outX > increaseW - 1 || outY > increaseH - 1 || outX < 0 || outY < 0) return;

    dst[outY * pitch + outX] = outV;
    dst[outY * pitch + outX + 1] = outV;
    dst[(outY + 1) * pitch + outX] = outV;
    dst[(outY + 1) * pitch + outX + 1] = outV;
}
...
__global__ void kawaseBlur(uint8_t* dst, uint8_t* src, size_t pitch, int offset, uint32_t width, uint32_t height)
{
    ...
    const int pox = bx + tx + offset; // plus offset
    const int poy = by + ty + offset;
    const int mox = bx + tx - offset; // minus offset
    const int moy = by + ty - offset;

    uint8_t sum = src[pox + pitch * poy] + src[pox + pitch * moy] + src[mox + pitch * poy] + src[mox + pitch * moy];
    uint8_t avg = ((sum >> 2) + src[srcX + pitch + srcY]) >> 1;

    dst[srcX + pitch * srcY] = avg;
}
...
__global__ void unsharp(int strength, uint8_t* dst, uint8_t* src, size_t pitch, uint8_t* origin, uint32_t width, uint32_t height)
{
    ...
    uint8_t oColor = origin[srcX + pitch * srcY];
    uint8_t bColor = src[srcX + pitch * srcY];

    uint8_t diff = oColor - bColor;
    uint8_t sharped = oColor + (strength * diff) / 100;

    if (sharped > 255 || sharped < 0) return;

    src[srcX + pitch * srcY] = (uint8_t)sharped;
}